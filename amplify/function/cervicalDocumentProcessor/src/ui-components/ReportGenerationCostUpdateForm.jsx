...
import { getReportGenerationCost } from "../graphql/queries";
import { updateReportGenerationCost } from "../graphql/mutations";
const client = generateClient();
export default function ReportGenerationCostUpdateForm(props) {
  const {
    id: idProp,
    reportGenerationCost: reportGenerationCostModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = reportGenerationCostRecord
      ? { ...initialValues, ...reportGenerationCostRecord }
      : initialValues;
    setOpenAiPromptCost(cleanValues.OpenAiPromptCost);
    setOpenAiCompletionCost(cleanValues.OpenAiCompletionCost);
    setAwsTextractCost(cleanValues.AwsTextractCost);
    setAwsComprehendMedicalCost(cleanValues.AwsComprehendMedicalCost);
    setTotalCost(cleanValues.TotalCost);
    setCreatedAt(cleanValues.createdAt);
    setDocumentKey(cleanValues.documentKey);
    setErrors({});
  };
  const [reportGenerationCostRecord, setReportGenerationCostRecord] =
    React.useState(reportGenerationCostModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getReportGenerationCost.replaceAll("__typename", ""),
              variables: { ...idProp },
            })
          )?.data?.getReportGenerationCost
        : reportGenerationCostModelProp;
      setReportGenerationCostRecord(record);
    };
    queryData();
  }, [idProp, reportGenerationCostModelProp]);
  React.useEffect(resetStateValues, [reportGenerationCostRecord]);
  const validations = {
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateReportGenerationCost.replaceAll("__typename", ""),
            variables: {
              input: {
                documentKey: reportGenerationCostRecord.documentKey,
                createdAt: reportGenerationCostRecord.createdAt,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "ReportGenerationCostUpdateForm")}
      {...rest}
    >
      <TextField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || reportGenerationCostModelProp)}
