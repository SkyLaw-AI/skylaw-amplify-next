...
from gpt_interface import assistant
# from gpt_interface.mri_instructions import get_entire_instructions
...
def handler(event, context):
    try:
        ...
        if http_method == 'POST':
            ...
            if pdf_bytes:
                try:
                    mri_processor = LumbarProcessor()
                    print('analyzing document')
                    mri_summary = mri_processor.generate_summary(pdf_bytes)

                    print('uploading db to s3')
                    s3_filepath = s3_interface.upload_mri_report_to_s3(bucket, key, mri_summary)

                    # uploading pii to s3
                    pii_filename = key.split("/")[-1].split('.')[0] + '_pii.json'
                    with open(f'/tmp/{pii_filename}', 'w') as fp:
                        json.dump(mri_processor.pii_entities, fp)
                    pii_s3_filepath = s3_interface.upload_file_to_s3(bucket, key, f'/tmp/{pii_filename}')
                    ...
                    try:
                        ...
                        invoice_s3_filepath = s3_interface.upload_file_to_s3(bucket, key, invoice_local_path)
                    ...
                    stripe_lib.update_stripe_usage(
                        1, stripe_customer_id, idempotency_key=key+"_cervical"
                    )
...
def get_invoice_details(invoice_cost, filename):
    ...
    invoice_details = {
        ...
        "mri_cost": f"${round(invoice_cost, 4)}",  # calculated internally
        "upload_time": details_from_dynamo['createdAt'],  # from dynamo
        "total_cost": f"${round(invoice_cost, 4)}", #  calculated internally,
...
class LumbarProcessor:
    def __init__(self) -> None:
        self.pricing_tracker_instance = pricing_tracker.PricingTracker()
        self.pii_entities = None

    def generate_summary(self, mri_filebytes):
        textract_ouput = self.analyze_document(mri_filebytes)
        all_text = ' '.join(page_dict['page_text'] for page_dict in textract_ouput)
        clean_output, pii_entities = self.remove_pii(all_text)

        self.pii_entities = pii_entities
        self.pricing_tracker_instance.update_comprehend_units(all_text)

        return self._generate_summary(clean_output)

    def _generate_summary(self, extracted_text):
        # entire_instructions = get_entire_instructions(mri_text)
        # use azure assistant
        assistant_inst = assistant.AzureOpenAIAssistant()
        run, thread = assistant_inst.generate_response(extracted_text)
        gpt_response, run, thread = assistant_inst.get_response(run, thread)
        
        # cost update
        self.pricing_tracker_instance.update_open_ai_tokens(run)

        return gpt_response.data[0].content[0].text.value
    ...
    def remove_pii(self, text):
        ...
        pii_entities['chunk_size'] = chunk_size
        for chunk in range(0, len(text), chunk_size):
            text_chunk = text[chunk:chunk + chunk_size]

            pii_entities_chunk = comprehendmedical.detect_phi(
                Text=text_chunk
            )

            if 'Entities' in pii_entities_chunk:
                for entity in pii_entities_chunk['Entities']:
                    text_chunk = text_chunk.replace(entity['Text'], '')
                final_text += text_chunk

                pii_entities[f'chunk_{chunk}'] = pii_entities_chunk['Entities']

        return final_text, pii_entities
